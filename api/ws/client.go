...
func NewClient(ctx context.Context, apiKey, secretKey, passphrase string, url map[bool]okex_sdk.BaseURL) *ClientWs {
...
func (c *ClientWs) Login() error {
	...
	return c.Send(true, okex_sdk.LoginOperation, args)
}
...
func (c *ClientWs) Subscribe(p bool, ch []okex_sdk.ChannelName, args map[string]string) error {
	count := 1
	if len(ch) != 0 {
		count = len(ch)
	}
	...
	return c.Send(p, okex_sdk.SubscribeOperation, tmpArgs)
}

// Unsubscribe into channel(s)
//
// https://www.okex.com/docs-v5/en/#websocket-api-unsubscribe
func (c *ClientWs) Unsubscribe(p bool, ch []okex_sdk.ChannelName, args map[string]string) error {
	...
	return c.Send(p, okex_sdk.UnsubscribeOperation, tmpArgs)
}

// Send message through either connections
func (c *ClientWs) Send(p bool, op okex_sdk.Operation, args []map[string]string, extras ...map[string]string) error {
	if op != okex_sdk.LoginOperation {
		err := c.Connect(p)
		if err == nil {
			if p {
